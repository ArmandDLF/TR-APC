...
from ._base import AbstractLinearOperator, square, diagonal

__all__ = [
    'FourierOperator',
    'HWPDemodOperator',
    'ChopperDemodOperator',
]
...
@diagonal
class HWPDemodOperator(AbstractLinearOperator):
    """Operator for demodulating HWP signal."""

    phase_func: callable = equinox.field(static=True)
    _in_structure: PyTree[jax.ShapeDtypeStruct] = equinox.field(static=True)
    sample_rate: float = equinox.field(static=True)
    n : int = equinox.field(static=True)
    phasor : Float[Array, '...']
    ...
    def __init__(
        ...
        *,
        ...
        self._in_structure = in_structure
        self.sample_rate = sample_rate
        self.n = in_structure.shape[-1]

        t_v = jnp.arange(self.n) / self.sample_rate
        phi_t = self.phase_func(t_v)
        self.phasor = jnp.exp(1j * phi_t)

    def mv(self, x: Float[Array, "n"]) -> Float[Array, "n"]:
        """Multiply TOD by phasor e^{i φ(t)}."""
        return x * self.phasor # Placeholder implementation

    def in_structure(self) -> PyTree[jax.ShapeDtypeStruct]:
        return self._in_structure

@diagonal 
class ChopperDemodOperator(AbstractLinearOperator):
    """Operator for demodulating chopper signal."""

    chopper_freq: float = equinox.field(static=True)
    chopper_phi: float = equinox.field(static=True)
    mode : str = equinox.field(static=True)
    _in_structure: PyTree[jax.ShapeDtypeStruct] = equinox.field(static=True)
    sample_rate: float = equinox.field(static=True)
    n : int = equinox.field(static=True)
    phasor: jnp.ndarray = equinox.field(static=True)
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            chopper_phi : phase offset [rad]
            in_structure: input structure (shape, dtype)
            sample_rate : sampling frequency [Hz]
            mode        : "square" (default) or "sin"
        """

        self.chopper_freq = chopper_freq
        self.chopper_phi = chopper_phi
        self.mode = mode
        self._in_structure = in_structure
        self.sample_rate = sample_rate
        self.n = in_structure.shape[-1]
        ...
        t_v = jnp.arange(self.n) / self.sample_rate
        if self.mode == "square":
            phase_cycles = (self.chopper_freq * t_v + self.chopper_phi / (2 * jnp.pi)) 
            frac = jnp.mod(phase_cycles, 1.0) 
            phasor = (frac < 0.5).astype(jnp.float32)
        elif self.mode == "sin":
            phasor = jnp.sin(2.0 * jnp.pi * self.chopper_freq * t_v + self.chopper_phi)
        else:
            raise ValueError(f"Invalid mode {self.mode}, choose 'square' or 'sin'")
        object.__setattr__(self, "phasor", phasor)

    def mv(self, x: Float[Array, "n"]) -> Float[Array, "n"]:
        """Multiply TOD by chopper phasor."""
        return x * self.phasor 
